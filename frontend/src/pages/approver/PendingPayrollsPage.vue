  ...
  <MainLayout>
    <div class="space-y-4">
      ...
      <div v-else class="overflow-x-auto">
        <table
          class="min-w-full bg-white shadow-md rounded-lg dark:bg-gray-800"
        >
          <thead
          ...
          <tbody>
            <tr
              ...
              :key="payroll.id"
              class="border-b hover:bg-gray-50 dark:border-gray-600 dark:hover:bg-gray-700"
            >
              <td class="px-4 py-2">{{ index + 1 }}</td>
              <td class="px-4 py-2">{{ payroll.month }}</td>
              <td class="px-4 py-2">
                Birr {{ payroll.total_amount.toLocaleString() }}
              </td>
              <td class="px-4 py-2">
                <span
                  class="text-yellow-600 font-semibold dark:text-yellow-400"
                >
                  {{ payroll.status }}
                </span>
              </td>
      ...
      >
        <div
          class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md dark:bg-gray-800 dark:text-gray-200"
        >
          <h2 class="text-lg font-semibold mb-4">Reject Payroll</h2>
          <textarea
            v-model="rejectionReason"
            placeholder="Enter reason for rejection..."
            class="w-full border rounded p-2 mb-4 dark:bg-gray-700 dark:border-gray-600"
            rows="4"
          ></textarea>
...
const submitRejection = async () => {
  ...
  try {
    ...
    payrolls.value = payrolls.value.filter(
      (p) => p.id !== selectedPayrollId.value
    );
